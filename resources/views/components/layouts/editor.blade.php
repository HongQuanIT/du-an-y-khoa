...
<body class="bg-surface-container-lowest font-body-md text-on-surface"
    x-data="{ menu: false, accountMenu: false, notificationsOpen: false, theme: 'light', initTheme() { this.theme = window.MedlearnTheme?.getStoredTheme?.() ?? 'system'; }, async setTheme(value) { this.theme = value; if (window.MedlearnTheme?.setTheme) this.theme = await window.MedlearnTheme.setTheme(value); } }"
    x-init="initTheme()" @keydown.escape.window="menu = false; accountMenu = false; notificationsOpen = false">
    ...
    <header class="fixed top-0 right-0 left-0 z-40 flex h-header-height items-center justify-between border-b border-outline-variant bg-surface px-margin-mobile md:left-sidebar-width md:px-margin-desktop">
        ...
        <div class="ml-2 flex shrink-0 items-center gap-2">
            @include('notification::partials.bell', ['indexRoute' => 'editor.notifications.index'])
            <div class="relative" @click.outside="accountMenu = false">
                <button type="button" @click="accountMenu = !accountMenu; notificationsOpen = false" class="flex items-center gap-3 rounded-xl p-1.5 text-left hover:bg-surface-container-low" :aria-expanded="accountMenu" aria-haspopup="dialog" aria-label="Mở menu tài khoản">
