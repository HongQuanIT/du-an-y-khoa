...
**Quan trọng:** `LIVEKIT_API_KEY` / `LIVEKIT_API_SECRET` phải **khớp** cặp trong [`docker/livekit/livekit.yaml`](../docker/livekit/livekit.yaml):

```yaml
keys:
  medlearn_dev_key: "medlearn_dev_secret_change_me"
```

| Biến | Ai dùng | Ghi chú |
|------|---------|---------|
| `LIVEKIT_URL` | Browser (qua token payload) | Host máy bạn → port publish. Local: `ws://localhost:7880` |
| `LIVEKIT_API_KEY` / `SECRET` | Laravel ký JWT | Phải trùng file yaml |
...
Copy key/secret vào **cả** `.env` và `docker/livekit/livekit.yaml`, rồi:
...
Browser không tới được địa chỉ ICE mà LiveKit quảng bá (thường là IP Docker `172.x`). Sửa bằng `rtc.node_ip: 127.0.0.1` trong `livekit.yaml` (đã set sẵn cho local), map cổng `7882/udp`, rồi:

```bash
docker compose up -d --force-recreate livekit
```

Mở app bằng `http://localhost:…` (không dùng IP LAN) khi `node_ip` là `127.0.0.1`.
...
Classroom tự start/stop Room Composite egress khi `LIVEKIT_EGRESS_ENABLED=true` và bucket S3/R2 đã cấu hình.
...
Local dev: để `LIVEKIT_EGRESS_ENABLED=false` — phòng ended hiển thị “không ghi hình” thay vì kẹt processing.
