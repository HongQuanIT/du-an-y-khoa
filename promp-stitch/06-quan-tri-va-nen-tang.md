...
Trạng thái loading skeleton widget. Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: chủ yếu xem KPI + cảnh báo, chart cuộn. Màu teal #0F766E, cảnh báo đỏ #DC2626/cam #D97706.
```
...
Khối chính: thanh tìm kiếm (email/tên) + bộ lọc (Vai trò, Trạng thái, Gói, Tổ chức); bảng người dùng mật độ cao mỗi dòng: checkbox, avatar + tên "Nguyễn Văn An", email, vai trò (badge Sinh viên/Giảng viên/Biên tập/Quản trị viên), gói (Premium/Miễn phí), trạng thái (Hoạt động xanh/Chờ/Tạm khóa/Cấm), lần đăng nhập cuối "2 giờ trước", menu hành động (...); sort theo cột; phân trang server "1–50 trong 38.240"; thanh hành động hàng loạt khi chọn nhiều (Đổi vai trò, Khóa, Xuất CSV).
...
- Tab Tổng quan: thông tin cá nhân, tổ chức, số liệu học.
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: tab cuộn. Màu teal #0F766E.
```
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: xem trước xuống dưới. Màu teal #0F766E.
```
...
Thanh tiến trình xử lý nền. Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: stepper dọc. Màu teal #0F766E.
```
...
Badge "Tự động lưu". Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: xem trước xuống dưới. Màu teal #0F766E.
```
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: 2 cột. Màu teal #0F766E.
```

### 37.2 Tải lên & Chi tiết media (`/admin/media/upload`, `/{id}`)
```
Hai màn hình Media của MedLuyện, App shell quản trị.
...
Khối chính: bộ lọc theo loại (Mô phỏng/Tự đánh giá/Đề tổ chức), trạng thái, lịch + tìm kiếm; nút "+ Tạo đề thi"; bảng đề mỗi dòng: tên "Mô phỏng thi Bác sĩ nội trú 2026", loại, số câu 150, thời lượng 180 phút, điểm đạt 70%, trạng thái, khoảng khả dụng "01/06–30/06/2026", số lượt làm, menu hành động (Sửa, Xuất bản, Giao bài, Xem kết quả).
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: bảng → thẻ. Màu teal #0F766E.
```
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: stepper dọc. Màu teal #0F766E.
```
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: KPI 2 cột, chart full, bảng cuộn. Màu teal #0F766E.
```
...
Khối chính:
(1) Danh sách vai trò dạng bảng: Sinh viên (38.100 user), Giảng viên (240), Biên tập nội dung (18), Quản trị tổ chức (32), Quản trị viên (8), Super Admin (2) — mỗi dòng có nút Sửa, Nhân bản, và khóa với vai trò hệ thống được bảo vệ; nút "+ Tạo vai trò".
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: ma trận cuộn ngang. Màu teal #0F766E.
```
...
Trạng thái loading skeleton. Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: bảng → thẻ. Màu teal #0F766E.
```

### 40.2 Chi tiết bản ghi (`/admin/audit/{id}`)
```
Drawer/màn hình chi tiết nhật ký (route /admin/audit/{id}) của MedLuyện, App shell quản trị.
Nội dung: tiêu đề hành động "Đổi vai trò người dùng"; metadata (Người thực hiện, Thời gian, IP, User-Agent, Mã request); khối so sánh "Trước / Sau" dạng diff hai cột tô màu (Trước: vai trò = Sinh viên; Sau: vai trò = Giảng viên); liên kết tới đối tượng liên quan; ghi chú không thể chỉnh sửa "Bản ghi bất biến".
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: 2 cột diff xếp dọc. Màu teal #0F766E, thêm #16A34A / bớt #DC2626.
```
...
Khối chính: lưới danh mục báo cáo dựng sẵn dạng thẻ có icon (Người dùng & tăng trưởng, Tương tác/engagement, Doanh thu & churn, Hiệu quả nội dung, Kết quả học tập, Báo cáo tổ chức); nút "Tạo báo cáo tùy biến"; khu "Báo cáo đã lên lịch" liệt kê báo cáo gửi email định kỳ.
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: lưới 1 cột. Màu teal #0F766E.
```

### 41.2 Báo cáo chi tiết & Trình tạo (`/admin/reports/{type}`, `/builder`)
```
Hai màn hình báo cáo của MedLuyện, App shell quản trị.
...
(2) Trình tạo báo cáo (route /admin/reports/builder): bố cục 2 cột — trái chọn Chỉ số (metric: tỷ lệ đúng, số câu, doanh thu...), Chiều (dimension: chủ đề, thời gian, tổ chức, gói), Bộ lọc/cohort; phải là vùng xem trước biểu đồ + bảng cập nhật realtime; nút "Lưu báo cáo" và "Lên lịch".
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: bộ lọc/cấu hình trên, kết quả dưới. Màu teal #0F766E.
```
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: block xếp dọc. Màu teal #0F766E.
```

### 42.2 Blog, FAQ, Banner, Menu (`/admin/cms/blog`, `/faq`, `/banners`, `/menus`)
```
Màn hình quản lý CMS phụ của MedLuyện, App shell quản trị, dạng nhiều mục quản trị.
...
Chọn 1 màn hình để dựng (ví dụ Banner). Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: 1 cột. Màu teal #0F766E.
```
...
```
Màn hình "Cổng lập trình viên (Developer Portal)" (route /developer), App shell quản trị/tổ chức.
Khối chính:
...
Tạo đồng thời 2 khung trong cùng màn hình — bản Desktop (bố cục đầy đủ như mô tả trên) và bản Mobile. Trên mobile: danh mục thành dropdown. Màu teal #0F766E, method GET xanh #16A34A / POST teal / DELETE đỏ #DC2626.
```
