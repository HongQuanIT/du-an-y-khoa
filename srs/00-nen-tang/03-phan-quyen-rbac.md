...
audit.view, report.view, report.export, cms.manage, media.view, media.manage, contact.view, contact.manage
notification_broadcast.view, notification_broadcast.send, system_setting.view, system_setting.update
...
| Admin | `content_editor`, `admin`, `super_admin` | CMS, user/RBAC, oversight, billing, `admin.partners.*`, `notification_broadcast.send`, … — **`question.publish` / `question.retire`: `admin` + `super_admin`**; **`question.submit` chỉ `content_editor`** |
...
|----------------------|-------|---------|
| `classroom.create` / `classroom.manage` (lớp mình) | **Instructor** (tạo trên `/teach`; lớp mới `pending_approval`) | Vận hành lớp mình host/cohost sau admin duyệt |
...
| `classroom_oversight.view_any` | Admin, Super Admin | Giám sát và force-end lớp — **không** thay workspace `/teach` |
| `user.role_assign` | Super Admin (Admin hạn chế) | Gán/thu hồi role hệ thống, gồm `instructor` |
