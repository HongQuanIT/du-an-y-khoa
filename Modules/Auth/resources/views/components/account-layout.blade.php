...
@php
    ...
    $isStudentAccount = auth()->user()?->hasRole(\App\Support\Enums\Role::Student->value) ?? false;
@endphp
...
@elseif ($isStudentAccount)
    <x-layouts.app :title="$accountTitle">
        <div class="mx-auto max-w-[1200px] space-y-6 p-margin-mobile md:p-margin-desktop">
            <div>
                <h1 class="font-headline-md text-headline-md text-on-surface">{{ $accountTitle }}</h1>
                @if ($accountDescription)
                    <p class="mt-1 text-body-md text-on-surface-variant">{{ $accountDescription }}</p>
                @endif
            </div>
