            ...
            <tbody>
                @forelse ($commissions as $commission)
                    <tr class="border-b border-outline-variant/60 last:border-0">
                        ...
                        <td class="px-4 py-3">{{ MoneyFormatter::vnd((int) $commission->gross_cents) }}</td>
                        <td class="px-4 py-3">{{ number_format($commission->rate_bps / 100, 1) }}%</td>
                        <td class="px-4 py-3 font-label-md">{{ MoneyFormatter::vnd((int) $commission->commission_cents) }}</td>
