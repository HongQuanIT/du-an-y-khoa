...
final class ContextualSearchTest extends TestCase
{
    ...
    {
        $secondaryTopic = $this->makeLesson([
            'name' => 'Truyền nhiễm',
            'slug' => 'truyen-nhiem-contextual-search-test',
            'sort_order' => 2,
        ]);
        $question = $this->question('Viêm phổi nhiễm khuẩn cần chọn kháng sinh', free: true);
        $question->lessons()->syncWithoutDetaching([$secondaryTopic->getKey()]);

        $result = app(SearchScopeAction::class)->handle(new SearchQueryData(
            scope: 'qbank',
            query: 'viêm phổi',
            filters: ['lesson_id' => $secondaryTopic->getKey(), 'is_free' => true],
        ), $this->user);
        ...
        $this->assertEqualsCanonicalizing(
            ...
            $result->items()[0]['attributes']['lesson_ids'],
        );
    }
    ...
    public function test_meilisearch_settings_define_filters_synonyms_and_after_commit(): void
    {
        ...
        $this->assertSame(['difficulty', 'lesson_ids', 'is_free'], $settings['filterableAttributes']);
    ...
    private function question(
        ...
        return tap(Question::factory()->create([
            ...
            $question->lessons()->sync([$this->topic->getKey()]);
        });
    }
}
