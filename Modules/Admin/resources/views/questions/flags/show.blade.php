<x-layouts.admin title="Review câu hỏi — {{ $question->code }}">
    <x-admin.page-header :title="'Review câu hỏi '.$question->code"
        description="Xem câu như học viên (gợi ý, kiến thức, giải thích đáp án) rồi gắn một cờ. Ghi chú tùy chọn — nên ghi khi cờ vàng hoặc đỏ.">
        <x-slot:actions>
            <a href="{{ route('admin.questions.flags.index') }}"
    ...
    <div class="grid grid-cols-1 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <div class="xl:sticky xl:top-4">
            @if ($canFlag)
                <form method="post" action="{{ route('admin.questions.flags.store', $question) }}"
                    class="rounded-2xl border border-outline-variant bg-surface p-5">
                    @csrf
                    <p class="mb-3 text-sm text-on-surface-variant">Chọn một cờ. Cờ đỏ sẽ chặn Admin xuất bản.</p>
