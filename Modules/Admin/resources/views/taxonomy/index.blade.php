<x-layouts.admin title="Phân loại câu hỏi">
    <x-admin.page-header title="Phân loại câu hỏi"
        description="Quản lý 3 lớp phân loại: Ma trận đề thi (128 chủ đề lâm sàng), Danh mục y khoa, Tags (metadata).">
    </x-admin.page-header>
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <x-admin.kpi-card label="Tags" :value="number_format($stats['tags'])" hint="Nhãn metadata" icon="sell" />
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <a href="{{ route('admin.tags.index') }}"
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <h3 class="font-label-lg font-semibold text-on-surface group-hover:text-primary">Tags</h3>
                    <p class="mt-1 text-sm text-on-surface-variant">
                        Nhãn metadata (ECG, Emergency, High-yield…) để lọc câu hỏi và custom session.
                    </p>
                    <p class="mt-3 text-xs font-semibold text-primary">Quản lý tags →</p>
                </div>
            </div>
        </a>
    </div>
