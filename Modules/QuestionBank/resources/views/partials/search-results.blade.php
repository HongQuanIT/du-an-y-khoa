@php
    ...
    $topicCounts = collect($searchResult->facets['lesson_id'] ?? [])->keyBy('value');
        ...
        <label class="block">
            ...
            <select name="filter[lesson_id]"
                ...
                @foreach ($searchTopics as $topic)
                    ...
                    @endphp
                    <option value="{{ $topic->id }}" @selected(($searchFilters['lesson_id'] ?? null) === $topic->id)>
    ...
    <div class="space-y-3">
        @foreach ($searchItems as $item)
            @php
                ...
                $topicIds = $attributes['lesson_ids']
                    ?? $attributes['topic_ids']
                    ?? array_values(array_filter([(int) ($attributes['lesson_id'] ?? 0)]));
                $topics = $searchTopics->only($topicIds);
                $sessionParams = array_filter([
                    'lesson_ids' => $topicIds ?: null,
