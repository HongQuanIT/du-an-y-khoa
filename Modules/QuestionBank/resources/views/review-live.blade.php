        ...
        <main class="custom-scrollbar min-w-0 flex-1 flex-col overflow-y-auto bg-white"
                    ...
                    <div class="mx-auto w-full max-w-4xl flex-1 space-y-8 p-4 pb-24 md:p-8 lg:p-10">
                        ...
                        <section x-show="current.note" class="space-y-3">
                            <div class="flex items-center gap-2">
                                ...
                                <h2 class="font-headline-sm text-headline-sm text-on-surface">Ghi chú của bạn</h2>
                            </div>
                            <p class="whitespace-pre-wrap rounded-xl border border-outline-variant bg-surface-container-lowest p-5 text-body-md text-on-surface"
                                x-text="current.note"></p>
                        </section>
                    </div>

                    <footer class="sticky bottom-0 z-20 flex items-center justify-between gap-3 border-t border-outline-variant bg-white/95 px-4 py-3 backdrop-blur md:px-6">
                        ...
                        <a href="{{ route('qbank.summary', $session) }}"
                            class="hidden rounded-lg border border-outline-variant px-4 py-2 text-sm font-bold text-primary hover:bg-primary/5 sm:inline-flex">
                            Về tổng kết
                        </a>
                        <button type="button" @click="move(1)" :disabled="!canMove(1)"
                            class="inline-flex items-center gap-1 rounded-lg bg-primary px-4 py-2 font-bold text-white disabled:cursor-not-allowed disabled:opacity-30">
            ...
            <template x-if="!current">
                <div class="flex h-full flex-col items-center justify-center p-8 text-center">
                    ...
                    <a href="{{ route('qbank.summary', $session) }}" class="mt-5 rounded-lg bg-primary px-5 py-2.5 font-bold text-white">Quay lại tổng kết</a>
                </div>
            </template>
        </main>
    </div>
