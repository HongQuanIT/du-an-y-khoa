@php
    /** @var \Illuminate\Support\Collection<int, \Modules\Classroom\Models\Classroom> $publicClassrooms */
    /** @var \Illuminate\Support\Collection<int, \Modules\Classroom\Models\Classroom> $myClassrooms */
    /** @var \Illuminate\Support\Collection<int, \Modules\Classroom\Models\Classroom> $liveNow */
    /** @var \Illuminate\Support\Collection<int, \Modules\Classroom\Models\Classroom> $upcomingSoon */
    /** @var list<int> $joinedClassroomIds */

    $filters = [
        '' => 'Tất cả',
        'live' => 'Đang live',
        'upcoming' => 'Sắp diễn ra',
    ];
@endphp

<x-layouts.app title="Lớp học">
    <div class="mx-auto max-w-[1200px] space-y-8 p-6 md:p-8">
        <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
            <div>
                <h1 class="font-headline-lg text-headline-lg text-on-surface">Lớp học</h1>
                <p class="mt-2 font-body-md text-body-md text-on-surface-variant">
                    Lớp chữa đề livestream — tham gia buổi live do giảng viên host.
                </p>
            </div>
        </div>

        @if (session('success'))
            <div class="rounded-xl border border-primary/20 bg-primary/10 px-4 py-3 text-sm text-primary">
                {{ session('success') }}
            </div>
        @endif

        <div class="flex flex-wrap gap-2">
            @foreach ($filters as $value => $label)
                <a href="{{ route('classroom.index', $value !== '' ? ['filter' => $value] : []) }}"
                    @class([
                        'rounded-full px-3 py-1.5 text-sm font-medium transition',
                        'bg-primary text-on-primary' => ($filter ?? '') === $value,
                        'bg-surface-container-low text-on-surface hover:bg-surface-container' => ($filter ?? '') !== $value,
                        'ring-2 ring-error/30' => $value === 'live' && ($filter ?? '') === 'live',
                    ])>
                    @if ($value === 'live')
                        <span class="mr-1 inline-block size-1.5 rounded-full bg-current align-middle"></span>
                    @endif
                    {{ $label }}
                </a>
            @endforeach
        </div>

        @if ($liveNow->isNotEmpty() && ($filter ?? '') !== 'upcoming')
            <section>
                <h2 class="mb-4 flex items-center gap-2 font-headline-sm text-headline-sm text-on-surface">
                    <span class="inline-flex size-2 rounded-full bg-error animate-pulse"></span>
                    Đang live
                </h2>
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($liveNow as $classroom)
                        @include('classroom::partials.card', [
                            'classroom' => $classroom,
                            'highlightLive' => true,
                            'joinedClassroomIds' => $joinedClassroomIds,
                        ])
                    @endforeach
                </div>
            </section>
        @endif

        @if ($upcomingSoon->isNotEmpty() && ($filter ?? '') === '')
            <section>
                <h2 class="mb-4 flex items-center gap-2 font-headline-sm text-headline-sm text-on-surface">
                    <span class="material-symbols-outlined text-[22px] text-primary">event</span>
                    Sắp live (7 ngày tới)
                </h2>
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($upcomingSoon as $classroom)
                        @include('classroom::partials.card', [
                            'classroom' => $classroom,
                            'joinedClassroomIds' => $joinedClassroomIds,
                        ])
                    @endforeach
                </div>
            </section>
        @endif

        @if (($filter ?? '') === '')
            <section>
                <h2 class="mb-4 font-headline-sm text-headline-sm text-on-surface">Lớp của tôi</h2>
                @if ($myClassrooms->isEmpty())
                    <p class="rounded-xl border border-dashed border-outline-variant bg-surface-container-low px-4 py-8 text-center text-on-surface-variant">
                        Bạn chưa tham gia lớp nào. Khám phá lớp công khai bên dưới.
                    </p>
                @else
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($myClassrooms as $classroom)
                            @include('classroom::partials.card', [
                                'classroom' => $classroom,
                                'isMember' => true,
                                'joinedClassroomIds' => $joinedClassroomIds,
                            ])
                        @endforeach
                    </div>
                @endif
            </section>
        @endif

        <section>
            <h2 class="mb-4 font-headline-sm text-headline-sm text-on-surface">
                @if (($filter ?? '') === 'live')
                    Lớp đang live
                @elseif (($filter ?? '') === 'upcoming')
                    Lớp sắp diễn ra
                @else
                    Lớp công khai
                @endif
            </h2>
            @if ($publicClassrooms->isEmpty())
                <p class="rounded-xl border border-dashed border-outline-variant bg-surface-container-low px-4 py-8 text-center text-on-surface-variant">
                    @if (($filter ?? '') !== '')
                        Không có lớp nào khớp bộ lọc này.
                    @else
                        Chưa có lớp công khai. Giảng viên tạo lớp trên portal /teach; admin duyệt trước khi hiển thị.
                    @endif
                </p>
            @else
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($publicClassrooms as $classroom)
                        @include('classroom::partials.card', [
                            'classroom' => $classroom,
                            'joinedClassroomIds' => $joinedClassroomIds,
                        ])
                    @endforeach
                </div>
            @endif
        </section>
    </div>
</x-layouts.app>
